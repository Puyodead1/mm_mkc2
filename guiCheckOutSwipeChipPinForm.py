...
# File: guiCheckOutSwipeChipPinForm.pyc (Python 2.5)
...
log = initlog('CheckOutSwipeChipPinForm')

class CheckOutSwipeChipPinForm(CustomerForm):
    
    def __init__(self):
        super(CheckOutSwipeChipPinForm, self).__init__()
        self.nextWindowID = 'CheckOutEjectForm'
        self.preWindowID = 'ShoppingCartForm'
        self.screenID = 'R12'
        self.lstResponseCtrl.extend([
            'CheckOutSwipeChipPinForm_ctr_message_box'])

    
    def _initComponents(self):
        super(CheckOutSwipeChipPinForm, self)._initComponents()
        self.timeoutSec = 60
    ...
    def _checkMemberDetail(self):
        ...
        if status == '0':
            ...
            while True:
                ...
                if ctrlID == 'CheckOutSwipeChipPinForm_ctr_message_box':
                    if self._getEventParam('CheckOutSwipeChipPinForm_ctr_message_box', 'val') != 'yes':
    ...
    def charge(self):
        msg = ''
        pm = { }
        (status, m) = self.upgProxy.chargeForChipNPin(self.customer, globalSession.shoppingCart)
        if status == '0':
    ...
    def on_CheckOutSwipeChipPinForm_ctr_message_box_event(self):
        if self._getEventParam('CheckOutSwipeChipPinForm_ctr_message_box', 'val') == 'yes':
            self.on_back()
    ...
    def _run(self):
        ...
        super(CheckOutSwipeChipPinForm, self)._run()
