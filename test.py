...
class SelfTest:
    ...
    def _getTestSlots(self, slots):  
        a = list(range(101, 171)) # Get 101 - 170 here
        b = list(range(228, 271))
        c = list(range(501, 571)) # Get 501 - 570
        d = list(range(601, 671))
        ...
        if slots:
            ...
            for ele in lstSlot:
                if str(ele).lower() in ["a", "b", "c", "d"]:
                    ...
                    lst = ele.split("-")
                    self.needTestSlotList += list(range(int(lst[0]), int(lst[1]) + 1))
        else:
